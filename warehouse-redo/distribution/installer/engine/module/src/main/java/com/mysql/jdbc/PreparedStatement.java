 ...
 * @version $Id: PreparedStatement.java,v 1.27.2.51 2005/04/07 23:55:57 mmatthews Exp $
 *
...
public class PreparedStatement extends com.mysql.jdbc.Statement
    ...
    public void setBigDecimal(int parameterIndex, BigDecimal x)
        ...
        if (x == null) {
            ...
            setInternal(parameterIndex, StringUtils.fixDecimalExponent(x.toString()));
        }
    }
    ...
    protected void setBytes(int parameterIndex, byte[] x, boolean checkForIntroducer, boolean escapeForMBChars)
        ...
        if (x == null) {
        	...
        	if (escapeForMBChars &&
        		this.connection.useUnicode() &&
        		connectionEncoding != null &&
        		CharsetMapping.MULTIBYTE_CHARSETS.get(
        				connectionEncoding.toLowerCase(Locale.ENGLISH)) != null) {
    ...
    public void setTimestamp(int parameterIndex, Timestamp x)
        ...
        setTimestampInternal(parameterIndex, x,
            this.connection.getDefaultTimeZone(), false);
    }
    ...
    private final void setInternal(int paramIndex, byte[] val)
        ...
        if ((paramIndex < 1)) {
            ...
            throw new SQLException("Parameter index out of range ("
                + paramIndex + " > number of parameters, which is " + (staticSqlStrings.length - 1) + ").",
                SQLError.SQL_STATE_ILLEGAL_ARGUMENT);
        }
    ...
    private synchronized void setTimestampInternal(int parameterIndex,
        ...
        if (x == null) {
            ...
            x = TimeUtil.changeTimezone(this.connection, x, tz,
                    this.connection.getServerTimezone(), rollForward);
    ...
    class ParseInfo {
        ...
        public ParseInfo(String sql, Connection conn,
            ...
            char[] sqlAsChars = sql.toCharArray();
            ...
            for (i = 0; i < statementLength; ++i) {
                ...
                if (!inQuotes && (i < stopLookingForLimitClause)) {
                    if ((c == 'L') || (c == 'l')) {
                        char posI1 = sqlAsChars[i + 1];

                        if ((posI1 == 'I') || (posI1 == 'i')) {
                            char posM = sqlAsChars[i + 2];

                            if ((posM == 'M') || (posM == 'm')) {
                                char posI2 = sqlAsChars[i + 3];

                                if ((posI2 == 'I') || (posI2 == 'i')) {
                                    char posT = sqlAsChars[i + 4];
            ...
            for (i = 0; i < staticSql.length; i++) {
                ...
                if (foundLoadData) {
                    String temp = new String(sqlAsChars, begin, len);
                    staticSql[i] = temp.getBytes();
                } else if (encoding == null) {
                    byte[] buf = new byte[len];

                    for (int j = 0; j < len; j++) {
                        buf[j] = (byte) sqlAsChars[begin + j];
                    }
                    ...
                    try {
                        if (converter != null) {
                            staticSql[i] = StringUtils.getBytes(sql, converter,
                                    encoding, connection.getServerCharacterEncoding(),
									begin, len,
                                    connection.parserKnowsUnicode());
                        } else {
                            String temp = new String(sqlAsChars, begin,
                                    len);
                            staticSql[i] = StringUtils.getBytes(temp, encoding,
