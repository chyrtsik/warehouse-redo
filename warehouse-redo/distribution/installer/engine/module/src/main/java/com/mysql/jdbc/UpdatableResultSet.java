...
public class UpdatableResultSet extends ResultSet {
    ...
    public synchronized void updateByte(int columnIndex, byte x)
        ...
        if (!onInsertRow) {
            ...
            this.thisRow[columnIndex - 1] = inserter.getBytes(columnIndex - 1);
        }
    }
    ...
    synchronized void syncUpdate() throws SQLException {
        ...
        for (int i = 0; i < numFields; i++) {
            if (thisRow[i] != null) {
                updater.setBytes(i + 1, thisRow[i], this.fields[i].isBinary(), false);
            } else {
        ...
        if (numKeys == 1) {
            ...
            updater.setBytes(numFields + 1, keyData, false, false);
        } else {
            for (int i = 0; i < numKeys; i++) {
                ...
                if (currentVal != null) {
                    updater.setBytes(numFields + i + 1, currentVal, false, false);
                } else {
