 ...
 */
public class VariantPriceComparator extends NumberComparator<Double> {

    /**
     * Possible string representation of the price (with special symbols, such as '<', '>', etc.)
     */
    private static final Pattern VARIANT_PRICE_STRING_PATTERN = Pattern.compile("^([<=>]{1,2})?[0-9]+([,|.][0-9]+)?$");
    private static final double VARIANT_PRICE_TRICK_STEP = 0.000000001;
    ...
    public int compareTo(Object o) {
        VariantPrice variantPrice = (VariantPrice) o;
        double comparableValue;
        if (variantPrice.isNumber()) {
            comparableValue = variantPrice.getPrice();
        } else {
            SearchItem searchItem = FilterParser.parse(StringUtils.removeSpaces(variantPrice.getValue()));
            ...
            if (searchItem.getSearchType() == SearchTypeConst.GREATER_SEARCH) {
                comparableValue += VARIANT_PRICE_TRICK_STEP;
    ...
    public boolean validate(Object value) {
        ...
        if (result) {
            VariantPrice variantPrice = (VariantPrice) value;
            if (!variantPrice.isNumber()) {
                result = StringUtils.containsSymbols(variantPrice.getValue())
                        && VARIANT_PRICE_STRING_PATTERN.matcher(StringUtils.removeSpaces(variantPrice.getValue())).matches();
            }
        }
        return result;
    }
